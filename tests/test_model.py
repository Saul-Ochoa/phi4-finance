...
def test_update_direction_is_gradient_ascent(monkeypatch):
    ...
    monkeypatch.setattr(m, "sampler", lambda n_chains=None: _FixedSampler(model_samples))
    mu0, lam0, a0 = m.mu.copy(), m.lam.copy(), m.a.copy()
    m.fit(data, method="ml", epochs=1, mcmc_steps=10, verbose=False)
...
def test_recovers_known_couplings(truth):
    m = Phi4Model(truth["V"], lr=0.1, mu_global=False, lam_global=False, seed=0, n_chains=8)
    m.fit(truth["data"], method="ml", epochs=250, mcmc_steps=30, batch_size=256, verbose=False)
...
def test_freeze_and_lambda_floor():
    ...
    mu0 = m.mu.copy()
    m.fit(data, method="ml", epochs=5, mcmc_steps=30, verbose=False)
...
def test_small_dataset_full_batch_and_history():
    ...
    m = Phi4Model(3, seed=0).fit(data, method="ml", epochs=3, mcmc_steps=20, batch_size=64, verbose=False)
    assert len(m.history) == 3 and {"m_data", "chi_model"} <= set(m.history[0])


def test_seed_reproducible():
    data = np.random.default_rng(0).uniform(-1, 1, (30, 3))
    a = Phi4Model(3, seed=7).fit(data, method="ml", epochs=3, mcmc_steps=20, verbose=False)
    b = Phi4Model(3, seed=7).fit(data, method="ml", epochs=3, mcmc_steps=20, verbose=False)
    assert np.array_equal(a.W, b.W)


def test_predict_conditional_units_and_shapes():
    m = Phi4Model(4, seed=0)
    out = m.predict_conditional({0: 0.1}, target_idx=3, n_samples=300, burn=20)  # MCMC path
